...
def generate_schema(profile: dict[str, Any]) -> str:
    ...
    # codegen with ``title`` as its merge key (see the dataclass + Neo4j
    # constraint blocks above), so it must always appear in the set even
    # when the source profile didn't define it.
